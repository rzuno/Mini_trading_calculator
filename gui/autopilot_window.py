...
the card gear system; the cards stay the V-Commandos worksheet for trading
by hand.
...
Layout is ONE information banner over TWO charts — nothing else:
    ...
    header   name · state · market phase · LIVE (grid-scale picker under it)
...
There are no manual Buy/Sell buttons and no dialogs of any kind: the LIVE
decision is an inline, modeless confirmation inside this window (a native
message box takes an application-wide grab on Windows and freezes the card
grid behind the cockpit), and a control failure prints on the status line.
When the army cannot fund the next buy level, the line is drawn muted
(✕ … no army) and the status says why.
...
class AutopilotWindow:
    ...
    def __init__(self, parent, ticker, currency, ap_ctx):
        ...
        # boxes take an application-wide grab on Windows and make the main
        # card window feel disabled while the commander compares its numbers.
        ...
        self._live_confirm_lbl = tk.Label(
            ...
            wraplength=1080, font=_F_SM, background='#FFF4E5')
    ...
    def _show_notice(self, message):
        ...
        self._status_lbl.config(text=str(message), fg=_STATUS_ERROR)
    ...
    def _on_update(self, ui):
        ...
        self._status_lbl.config(
            text=(f"⚠ {alert}" if alert else
                  f"{ui.get('status', '')}    poll {ui.get('ts', '--')}"),
            fg=(_STATUS_ERROR if alert else _STATUS_NEUTRAL))
